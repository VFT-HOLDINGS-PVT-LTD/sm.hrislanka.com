...
class Leave_Approve extends CI_Controller {
    ...
    public function approve($ID) {
        ...
        $system_page_name = "Leave_Transaction - Leave Approve Admin";//change action
        $spnID = $this->Db_model->getfilteredData("select `id` from tbl_audit_pages where `system_page_name` = '".$system_page_name."'");

        $dataArray = array(
            'log_user_id' => $Emp,
            'ip_address' => $ip,
            'system_action' => 'Leave is Approved. Its ID is '.$ID.'',//change action
...
//                    var_dump($Balance_Usd);die;
        ...
        $dataArray = array(
            'log_user_id' => $Emp,
            'ip_address' => $ip,
            'system_action' => 'Leave is Rejected. Its ID is '.$ID.'',//change action
            'trans_time' => $current_time,
            'system_page' => $spnID[0]->id 
        );
