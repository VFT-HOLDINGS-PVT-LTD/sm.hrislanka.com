...
class Attendance_Manual_Entry extends CI_Controller {
    ...
    public function index() {
        ...
        $data['data_set_att'] = $this->Db_model->getfilteredData("SELECT `M_ID`,`EmpNo`,`Emp_Full_Name`,`Att_Date`,`In_Time`,`tbl_manual_entry`.`Status`,`Reason`,`Is_Cancel`,`Is_App_Sup_User`,`Is_Admin_App_ID` from tbl_manual_entry inner join tbl_empmaster on tbl_empmaster.EmpNo = tbl_manual_entry.Enroll_No order by M_ID desc");
    ...
    public function emp_manual_entry() {
        ...
        $insert_id = $this->Db_model->getfilteredData("SELECT `M_ID` FROM tbl_manual_entry WHERE `Att_Date`='".$att_date."' AND `Enroll_No`='".$emp."'");//change action
        ...
        $system_page_name = "Attendance - Manual Attendance Entry";//change action
        $spnID = $this->Db_model->getfilteredData("select `id` from tbl_audit_pages where `system_page_name` = '".$system_page_name."'");

        $dataArray = array(
            'log_user_id' => $Emp,
            'ip_address' => $ip,
            'system_action' => 'A new manual attendance entry has been added. Its ID is '.$M_ID.' and Its have these '.$emp.','.$att_date.','.$in_time.','.$reason.','.$st.' details.',//change action
        ...
        $dataArray = array(
            'log_user_id' => $Emp,
            'ip_address' => $ip,
            'system_action' => 'A new manual attendance entry has been added. Its ID is '.$M_ID.' and Its have these '.$EnrollNo.','.$att_date.','.$in_time.','.$reason.','.$st.' details.',//change action
            'trans_time' => $current_time,
            'system_page' => $spnID[0]->id 
        );
