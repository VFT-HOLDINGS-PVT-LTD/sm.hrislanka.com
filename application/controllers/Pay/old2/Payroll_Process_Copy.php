...
class Payroll_Process extends CI_Controller
{
    ...
    public function emp_payroll_process()
    {
        ...
        for ($x = 0; $x < count($dtEmp['EmpData']); $x++) {
            ...
            $HasmsRow = $this->Db_model->getfilteredData("select COUNT(tbl_individual_roster.EmpNo) AS misspunch from tbl_individual_roster where EmpNo='$EmpNo' 
            and EXTRACT(MONTH FROM FDate)='$month' and EXTRACT(YEAR FROM FDate)='$year' AND DayStatus = 'MS'");
            // echo $EmpNo;
            // echo "<br/>";
            // echo $HasmsRow[0]->misspunch;
            // echo "<br/>";
            // echo "<br/>";
            // echo "<br/>";
            if ($HasmsRow[0]->misspunch == 0) {
                ...
                if ($HasRow[0]->HasRow > 0) {
                    ...
                    $attbchance = 3;
                    $rounded_number = 0;
                    $latealldata = $this->Db_model->getfilteredData("select InTime,LateM,InDate from tbl_individual_roster where EmpNo=$EmpNo and EXTRACT(MONTH FROM FDate)=$month and RYear=$year AND ShType = 'DU' ");
                    $late10adu0tawadi = $this->Db_model->getfilteredData("select COUNT(tbl_individual_roster.EmpNo) count from tbl_individual_roster where EmpNo='$EmpNo' and EXTRACT(MONTH FROM FDate)=$month and RYear=$year AND ShType = 'DU'
                                                                                        AND (tbl_individual_roster.LateM < 10 AND tbl_individual_roster.LateM >0)");
                    $late10tawadi = $this->Db_model->getfilteredData("select COUNT(tbl_individual_roster.EmpNo) AS count from tbl_individual_roster where EmpNo='$EmpNo' and EXTRACT(MONTH FROM FDate)=$month and RYear=$year AND ShType = 'DU'
                                                                                         AND tbl_individual_roster.LateM > 10");

                    if ($late10adu0tawadi[0]->count > 3 || $late10tawadi[0]->count) {
                        $attbchance = 0;
                    }
                        ...
                        if ($ld->LateM > 0) {
                            ...
                            if ($ld->LateM <= 10 && $ld->LateM > 0) {
                                ...
                                if ($attbchance <= 0) {
                                    ...
                                    // echo "/ Date " . $ld->InDate;
                                    // echo "/ Late minut " . $ld->LateM;
                                    // echo " /Late amount " . $Late_rate * 1;
                                    // echo "<br/>";
                                ...
                                // echo "round".$rounded_number;
                                $Late_Amount = ($Late_rate * $rounded_number) + $Late_Amount;
                                // echo "/ Date " ." ". $ld->InDate;
                                // echo "/ Late minut " ." ". $ld->LateM;
                                // echo "/ Late amount " ." ".$Late_rate * $rounded_number;
                                // echo "<br/>";
                                $attbchance = 0;
                            ...
                            $late_data[] = array(
                                ...
                                'InDate' => $ld->InDate,
                                'LateM' => $ld->LateM,
                                'Late_Amount' => $Late_rate * $rounded_number
                            );
                        }
                    }

                    foreach ($late_data as $ld) {
                        // echo $ld['EmpNo'];
                        // echo "<br/>";
                        // echo $ld['InDate'];
                        // echo "<br/>";
                        ...
                        $this->db->insert_batch('tbl_late_amount', $data);
                    }
                    ...
                    $Gross_sal = $BasicSal + $Incentive + $Fixed_Allowance + $budgetrelevances;
        ...
        // Log_Insert - Start
            ...
            $system_page_name = "Payroll - Payroll Process";//change action
            $spnID = $this->Db_model->getfilteredData("select `id` from tbl_audit_pages where `system_page_name` = '".$system_page_name."'");

            $dataArray = array(
                'log_user_id' => $Emp,
                'ip_address' => $ip,
                'system_action' => 'Payroll Process successfully. It processed to this '.$month.' month and '.$year.' year.',//change action
