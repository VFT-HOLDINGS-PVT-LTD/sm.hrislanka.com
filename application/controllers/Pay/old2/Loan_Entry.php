...
class Loan_Entry extends CI_Controller {
    ...
    public function insert_data() {
        ...
        if ($HasRow->HasRow>0) {
            ...
            $system_page_name = "Payroll - Loan Entry";//change action
            $spnID = $this->Db_model->getfilteredData("select `id` from tbl_audit_pages where `system_page_name` = '".$system_page_name."'");

            $dataArray = array(
                'log_user_id' => $Emp,
                'ip_address' => $ip,
                'system_action' => 'A new loan has been added (Employee cannot apply new loan untill settle loans)',//change action
...
//                    'Paid_Amount' => $month,
        ...
        // Log_Insert - Start
            ...
            $dataArray = array(
                'log_user_id' => $Emp,
                'ip_address' => $ip,
                'system_action' => 'A new loan has been added. Its ID is '.$loan_id.' and Its have these '.$EmpData[$i]->EmpNo.','.$amount.','.$timestamp.','.$No_of_Inst.','.$Rate.','.$Month_Inst.','.$Full_Amount.','.$month.','.$year.' details',//change action
                'trans_time' => $current_time,
                'system_page' => $spnID[0]->id 
            );
