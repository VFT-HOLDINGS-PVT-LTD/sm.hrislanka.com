...
//        $IsAllowance = $this->Db_model->getfilteredData("select count(EmpNo) HasRow from tbl_varialble_allowance where EmpNo ='$Emp' and Alw_ID='$allowance' and month='$month' and Year = '$year' ");
        ...
        //  $Day_type = $this->input->post('cmb_day');
         ...
         $system_page_name = "Payroll - Salary Increment";//change action
         $spnID = $this->Db_model->getfilteredData("select `id` from tbl_audit_pages where `system_page_name` = '".$system_page_name."'");
 
         $dataArray = array(
             'log_user_id' => $Emp,
             'ip_address' => $ip,
             'system_action' => 'A Salary Increment has been added. Its have these '.$Category.','.$Selected_Category.','.$amount.','.$month.' details',//change action
